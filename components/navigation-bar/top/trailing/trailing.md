...
**⚠️ 맨 끝이 Button이면 우측 패딩이 `spacing/11`(20px)입니다** (디자이너 지시, 2026-09-15 — Figma는 12px 고정).

아이콘은 [Icon Button](../../../button/icon-button/icon-button.md) L(40×40) 안에 24px 글리프가 가운데 놓여 **좌우 8px이 남습니다.** 그래서 12(묶음 패딩) + 8(버튼 안쪽) = **글리프가 이미 화면 끝에서 20px** 안쪽에 서 있습니다. 반면 [Text Button](../../../button/text-button/text-button.md)은 패딩이 0이라 글자가 상자 끝까지 차서 12px밖에 남지 않아, 같은 자리에 놓여도 **아이콘보다 8px 더 튀어나와 보입니다.** 맨 끝이 버튼일 때만 패딩을 20px로 올려 둘을 같은 자리에 세웁니다.

| 맨 끝 항목 | 묶음 우측 패딩 | 화면 끝 ↔ 보이는 것 |
|---|---|---|
| 아이콘 | `spacing/08`(12px) | **20px** (12 + Icon Button 안쪽 8) |
| 버튼 | **`spacing/11`(20px)** | **20px** (Text Button은 패딩 0) |

이 값은 묶음 전체가 갖습니다 — 버튼 항목에 margin으로 붙이면 아이콘과 맞닿는 쪽의 8px(바로 아래 규칙)과 겹쳐 계산이 꼬입니다. 끝에서 재는 값이라 묶음이 갖는 편이 맞습니다.
...
실측: Icon+Button 묶음 = 40 + 8 + 31("완료") + **20** = **99px**. 아이콘 2개 + 버튼 = **139px**.
(끝의 20px은 맨 끝이 버튼이라서입니다 — 아이콘으로 끝나면 12px이고 크기는 위 표의 52/92/132px 그대로입니다.)
...
- **맨 끝이 Button이면 우측 패딩이 20px**(`spacing/11` → `ref-spacing-11`) — 아이콘이 Icon Button 안쪽 여백 8px 덕에 이미 끝에서 20px 안쪽이라, 버튼도 같은 자리에 서도록 맞춘 것입니다. Figma는 12px 고정이므로 **이 항목은 구현이 Figma보다 앞서 있습니다** — 실측으로 되돌리지 마세요(디자이너 지시, 2026-09-15, 2장)
