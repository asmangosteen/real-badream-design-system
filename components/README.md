...
| Top Bar | [components/navigation-bar/top-bar/top-bar.md](./navigation-bar/top-bar/top-bar.md) | 완료 | 10개 인스턴스 (Mode Light/Dark × Background No/Default/Blur × Line Off/On, 비직교 — Background=No엔 Line 없음, 사용자 확인). Status Bar + Navigation Bar Top을 세로로 쌓은 최종 합성 컴포넌트. 배경 3종(없음/채움/블러), Status Bar·Navigation Bar 개별 hide 가능(둘 다 사용자 확인). 6개 노드 전략적 샘플링(0장 참고) |
...
`components/navigation-bar/` 폴더는 상단/하단 내비게이션 바 컴포넌트 패밀리를 모아두는 곳입니다. `components/date-time-picker/`와 동일하게, 화면 위치별로 하위 폴더를 나눕니다 — 지금은 [`top/`](./navigation-bar/top/top/top.md)(상단 내비게이션 바)와 [`top-bar/`](./navigation-bar/top-bar/top-bar.md)(Status Bar까지 포함한 최종 합성)가 존재하며, Bottom Navigation Bar가 추가되면 `components/navigation-bar/bottom/`에 별도로 문서화될 예정입니다.
...
한 단계 더 위에는 [`top-bar/`](./navigation-bar/top-bar/top-bar.md)가 있습니다 — [Status Bar](./status-bar/status-bar.md)(별도 컴포넌트 패밀리)와 [Navigation Bar Top](./navigation-bar/top/top/top.md)을 세로로 쌓아 화면 최상단에 그대로 얹는 **최종 합성 컴포넌트**입니다. 배경 처리 3종(없음/단색/블러), Line(구분선) 유무, Status Bar·Navigation Bar 개별 hide 등 Top Bar 전용 규칙이 있습니다(top-bar.md 참고).
