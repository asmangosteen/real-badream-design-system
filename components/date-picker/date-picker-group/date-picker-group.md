...
- 즉 좌우 두 패널이 [Calendar Header](../calendar-header/calendar-header.md)의 서로 다른 두 변형(좌="With=Nothing"+커스텀 드롭다운 토글, 우="With=Arrows")을 나눠 쓰는 비대칭 구조입니다 — 범위 선택기에서 월 이동(Arrow Box)은 한쪽에만 두고, 빠른 연/월 점프(드롭다운 토글)는 반대쪽에만 두는 설계로 추정됩니다. **확인 필요**(사용자에게 의도 확인 권장).
...
- 두 Date Picker 패널이 실제로 "범위의 시작/끝"을 나타낸다면 `role="group"` + `aria-label`(예: "시작일 선택"/"종료일 선택")로 각 패널을 구분하는 마크업이 필요해 보이나 Figma 파일에 규정 없음 — 확인 필요.
- Horizontal의 비대칭 헤더 구조(한쪽만 월 이동, 한쪽만 연/월 드롭다운)가 실제 상호작용에서 두 패널의 날짜를 동기화해서 이동시키는지(예: 좌측에서 드롭다운으로 점프하면 우측도 다음 달로 자동 이동하는지)는 디자인만으로 확인 불가 — 확인 필요.
...
**확인 필요**
- Horizontal 비대칭 헤더 구조의 실제 상호작용 의도(2장)
- 두 패널의 날짜 동기화 여부
- Picker Count=3류의 Time Picker 통합 여부(Date Picker Group 자체에는 Time Picker 섹션이 관찰되지 않음 — Time Picker=Off 조합만 샘플링됨)
