...
**중요**: 3개 State 전부 텍스트 색상이 `neutral/800`로 동일합니다 — Text Input의 Placeholder(옅은 회색)처럼 값이 없을 때 색을 죽이는 처리가 없습니다. 즉 이 컴포넌트에서 샘플링된 "00"은 **항상 실제 값처럼 렌더링되는 표시**로 보이며, 별도의 "빈 값/placeholder" 변형은 Figma 컴포넌트 자체에 존재하지 않습니다 — 확인 필요(7장).
...
4. **Disabled 상태가 Figma 컴포넌트에 존재하지 않습니다.** Default/Hover/Typing 3종뿐이며, 비활성화(선택 불가) 표현이 필요하다면 이 컴포넌트가 제공하는 변형 밖의 별도 처리가 필요해 보입니다 — 확인 필요.
...
- **Time Picker가 이 컴포넌트를 그대로 인스턴스로 재사용**합니다 — [`time-picker.md`](../time-picker/time-picker.md) 참고. 단, Time Picker 자체의 병합 변형 코드 안에서는 내장된 Time Field가 **State=Default로 고정**되어 있어(Hover/Typing 옵션이 코드 타입에서 아예 제외됨), Time Picker 안에서 Time Field의 Hover/Typing이 실제로 어떻게 나타나는지는 이번 조사로 확인하지 못했습니다 — 확인 필요.
...
- Disabled 변형이 없어 비활성화가 필요한 화면에서 대체 처리 방식이 필요 — 확인 필요(4장).
...
- Disabled 상태가 없는 것에 대한 대체 처리 규칙 없음
