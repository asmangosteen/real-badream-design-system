...
| 콘텐츠 너비 | — | 부모 폭 − 좌우 20px×2 (Figma 진열 390px 기준이면 350px) |
...
**Footer 너비는 고정이 아니라 화면(부모) 폭에 맞춰 늘어나는 가변입니다** (2026-09-14 디자이너 확정). Figma 진열 프레임의 390px 는 iPhone 14 기준 화면 폭을 보여준 것이고, 콘텐츠 너비 350px 은 거기서 좌우 20px 마진을 뺀 결과값입니다(저장소 `docs` 그리드 규칙과 일치). 즉 Divider 와 같은 방식으로, 좌우 20px 마진만 고정이고 콘텐츠는 남는 폭을 모두 차지합니다. 다만 **홈 화면 전용이라는 제약(1장)은 그대로**이므로 다른 화면에 재사용하지는 않습니다.
