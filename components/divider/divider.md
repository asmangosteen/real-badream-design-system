...
실측한 8개 노드 모두 컨테이너 너비가 390px로 렌더링되어 있으나(내부 `Rectangle`은 `flex-[1_0_0]`로 부모를 꽉 채우도록 설정), 이 390px는 **고정값이 아니라 Figma 컴포넌트 셋 진열 프레임(`2041:2838`, 전체 폭 430px에 좌우 20px 여백)의 크기를 그대로 보여준 것**입니다. **실제 구현 시 Divider는 부모 컨테이너 폭(화면 폭)에 맞춰 늘어나는 `width: 100%` 설계입니다.**
...
- 너비 `100%`(부모 컨테이너/화면 폭에 맞춰 늘어남, 3장 참고)
