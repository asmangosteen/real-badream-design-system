...
### 2-1. 아이콘은 두 슬롯이 서로 다른 계단을 탑니다 (✅ 의도된 설계)
...
> **✅ 2026-09-16 디자이너 확인 — 의도된 설계입니다. 다시 지적 사항으로 올리지 마세요.**
> "Angular 아이콘이 20px 박스에서도 16px로 고정인 것"은 오타나 누락이 아니라 의도한 값입니다.
> 구현도 이 값을 그대로 따릅니다(`ICON_SIZE` 테이블 — 파생식이 아니라 실측 명시 테이블입니다).
...
|---|---|---|---|
| Angular 박스 16px | 4px (`radius/02`) | 60% | 적용 |
| Angular 박스 20px | 6px (`radius/03`) | 60% | 적용 |
| Rounded 박스 | 999px (`radius/12`) | 60% | 변화 없음(원) |

- 미선택 테두리(1px)도 같은 squircle 을 따릅니다. 예전 구현은 진짜 `border` 였는데 레이어로 옮겼습니다 — 박스가 고정 크기라 바깥 크기·체크 위치는 그대로입니다.
