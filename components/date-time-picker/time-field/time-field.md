...
- `get_motion_context`를 패밀리 최상위 그룹(`2497:13877`, recursive=true)에 호출한 결과는 `{"nodes":[]}`였습니다(오케스트레이터 사전 확보, 패밀리 전체 11개 컴포넌트 공용). **다만 이건 모션이 없다는 뜻이 아니었습니다** — 반응 2건이 실제로 있습니다. 6장 참고(2026-09-15 정정).
...
|---|---|---|---|
| Time Field | 12px (`radius/06`) | 60% | 적용 |
| Typing 캐럿(Text Blinker) | 2px | 60% | 변화 없음(폭 2px) |

- Typing 의 파란 테두리도 같은 squircle 을 따릅니다. 키 입력을 받는 투명 input 은 레이어 위에 있어 동작이 그대로입니다.
