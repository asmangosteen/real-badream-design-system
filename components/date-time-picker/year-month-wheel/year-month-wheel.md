...
- `get_motion_context`는 패밀리 최상위 그룹(`2497:13877`, recursive=true)에서 이미 확인되어 모션 데이터 없음이 확정되어 있습니다(재호출하지 않음, 5장 참고). ~~휠 스크롤 자체의 관성/스냅 애니메이션이 이 컴포넌트의 핵심 인터랙션일 것으로 예상되지만 Figma에 모션 데이터가 없어 확인 필요입니다.~~ → **2026-09-15 해소.** 디자이너 지시로 "마우스 스크롤로 휠이 돌아가고, 애플과 비슷한 애니메이션"으로 확정했습니다. 구현은 **네이티브 스크롤 + `scroll-snap-type: y mandatory`** 를 그대로 씁니다 — 트랙패드의 관성·감속이 브라우저 것이라 따로 흉내 낼 필요가 없고, 멈추면 항목에 달라붙습니다(9장 참고).
...
### 9-1. 거리별 중심 오프셋 — 0 / 32 / 60 / 84px

4장은 행 사이 gap 만 기록했는데, 실제로 필요한 값은 **선택 행 중심에서 각 행 중심까지의 거리**입니다.
노드 좌표로 다시 재면 `0 / 32 / 60 / 84px` 이고, 간격이 **32 → 28 → 24 로 4씩 줄어듭니다.**
바깥으로 갈수록 좁아지는 이 패턴이 "휠이 휘어 보이는" 인상을 만듭니다.

원통 공식(`R·sin(d·α)`)으로 맞춰 보면 앞의 두 값(32·60)은 정확히 떨어지지만 세 번째가 80.5 로
84 와 어긋납니다. 손으로 다듬은 값이라 **실측 정지점을 그대로 쓰고 사이만 이어 붙입니다.**
스크롤 중간 위치에서도 끊기지 않게 하려면 이 보간이 필요합니다.

| 거리 | 중심 오프셋 | 타이포 | 투명도 |
|---|---|---|---|
| 0 | 0 | SubTitle/18 **M** | 100% |
| 1 | 32 | SubTitle/18 R | 70% |
| 2 | 60 | Body 1/16 R | 50% |
| 3 | 84 | Body 2/14 R | 30% |

### 9-2. 텍스트 정렬 — 연도 가운데, 월 왼쪽

`textAlignHorizontal` 실측값입니다. 연도는 80px 폭에 **CENTER**, 월은 40px 폭에 **LEFT** 이고
두 컬럼 사이 gap 은 12px, 묶음 전체가 행 안에서 가운데 정렬됩니다.

### 9-3. 월 열은 감깁니다

진열 샘플에서 1월 **위에** 10·11·12월이 보이는 것이 근거입니다. 월 열은 12개 목록을
여러 벌 이어 붙여 무한히 감기게 하고, 스크롤이 멈출 때마다 가운데 벌로 되돌립니다
(같은 목록이라 눈에 보이지 않습니다). **연도 열은 감기지 않습니다** — 범위가 있는 목록입니다.

⚠️ 연도 목록의 범위는 **선택값과 무관하게 고정**해야 합니다. 범위를 현재 선택 연도에서
파생시키면(예: `year−60 … year+20`) 연도를 고를 때마다 목록 전체가 밀려서 같은 연도의
위치가 달라지고 스크롤이 엉뚱한 칸으로 튑니다.

### 9-4. 3장 "핵심 발견 2" 를 구현이 따르지 않고 있었습니다

문서에는 **"연/월은 독립된 두 개의 휠(사용자 확인 완료)"** 로 맞게 적혀 있었는데,
