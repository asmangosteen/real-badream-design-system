...
| 축(Axis) | 값 | 의미 |
|---|---|---|
> **2026-09-15 — 4주짜리 달도 존재하며, 그대로 4줄로 그립니다.**
> 그 달이 **정확히 28일이고 일요일에 시작**하면 4개 행으로 끝납니다(가장 가까운 예: **2026년 2월**).
> 평년 2월에서만 나오고, 윤년이나 29~31일 달에서는 생기지 않습니다.
>
> Figma 에는 `Week Number=4` 변형이 없지만 **굳이 추가하지 않기로 했습니다**(디자이너 확인).
> 구현은 빈 줄로 5주를 맞추지 않고 그 달이 차지하는 만큼만 그립니다
> (`storybook/src/components/Calendar/calendar-data.ts`). 따라서 그런 달에는 패널이 그만큼 짧아집니다.
>
> | 주 수 | Month 높이 | Date Picker 패널 높이 |
> |---|---|---|
> | 4주 | **178px** (40×4 + 6×3) | 288px |
> | 5주 | 224px (40×5 + 6×4) | 334px |
> | 6주 | 270px (40×6 + 6×5) | 380px |
...
## 2.1 기간(range) 띠 — Month가 직접 그리는 유일한 도형

`2612:16030`의 세 번째 예시에서 Month는 Week 행들 **뒤에 사각형을 깝니다**. Figma 실측 7개 전수:

| 노드 | 소속 Month | x | y | w | h | 채움 |
|---|---|---|---|---|---|---|
| `2612:15600` | 그룹 왼쪽 패널 | 116 | 46 | **212** | 40 | `brand/primary-lightest`(#eef4fc) |
| `2612:15602` | 〃 | 0 | 92 | **328** | 40 | 〃 |
| `2612:15603` | 〃 | 0 | 138 | **328** | 40 | 〃 |
| `2612:15604` | 〃 | 0 | 184 | **328** | 40 | 〃 |
| `2612:15605` | 그룹 오른쪽 패널 | 0 | 0 | **328** | 40 | 〃 |
| `2612:15607` | 〃 | 0 | 46 | **164** | 40 | 〃 |
| `2612:15620` | 단독 Date Picker | 116 | 46 | **192** | 40 | 〃 |

- **모서리 radius 0** — Date 셀(999px 원)과 달리 각진 사각형입니다.
- **z-순서상 항상 Week보다 뒤**입니다(`children` 배열에서 사각형들이 Week 인스턴스보다 앞 = 아래 레이어).
- 채움색이 [Date](../date/date.md) `Type=Selected`의 배경과 **같은 변수**라 원과 띠의 경계가 보이지 않습니다.

### 왜 필요한가

Selected 셀은 지름 40px의 **원**인데 칸 사이에 `spacing/06`(8px) 간격이 있어, 원만으로는 기간이 8px씩
끊긴 점선처럼 보입니다. `2612:15933` 주석이 *"selected 항목 뒤에 동일한 배경색이 기간 사이를 채움"*이라고
적어 둔 게 이 사각형입니다.

### 양끝 규칙 (위 7개 실측에서 역산, 전부 일치)

한 줄에 사각형 하나입니다. 좌우 끝을 각각 이렇게 정합니다:

| 그 줄의 상황 | 좌/우 끝 |
|---|---|
| 기간의 **앵커(Pinned)가 그 줄에 있다** | 그 칸의 **중앙** (`48×칸번호 + 20`) |
| 기간이 그 줄을 **지나간다** | 줄 끝까지 (`0` 또는 `328`) |

칸 간격 48px = 40(셀) + 8(gap)입니다. 검산:

- `2612:15600` — 앵커가 2번 칸: `48×2+20 = 116` → 오른쪽은 지나감 `328`. 폭 **212** ✓
- `2612:15607` — 왼쪽은 지나옴 `0` → 앵커가 3번 칸: `48×3+20 = 164`. 폭 **164** ✓
- `2612:15620` — 앵커 2번·6번 칸: `116 → 48×6+20 = 308`. 폭 **192** ✓
- `2612:15602~5` — 양쪽 다 지나감: `0 → 328`. 폭 **328** ✓

앵커 칸은 Pinned(진한 파란 원)라, 띠가 **원의 중앙에서 뻗어 나오는 것처럼** 보입니다.

### 달의 빈 칸(Null)을 만났을 때

Figma 예시는 35칸이 전부 "00"으로 차 있어 이 경우가 없습니다. 구현에서는 **실제 날짜가 있는 칸까지만**
칠합니다 — 달의 마지막 줄이 `[27 28 29 30 · · ·]`이면 띠는 30일 칸의 오른쪽 변에서 끝납니다.
빈 칸 위를 지나가지 않는 쪽이 자연스럽다고 보고 정한 것입니다. → 확인 필요

### 구현에서의 위치 차이

Figma는 사각형을 **Month 프레임의 자식**으로 두었지만(Week가 인스턴스라 그 안에 넣을 수 없음),
구현에서는 **Week 안**에 넣었습니다 — 좌표가 어차피 그 줄 안에서만 결정되기 때문입니다.
그리는 결과는 같습니다. 규칙은 `Calendar/calendar-data.ts`의 `weekRangeBand()` 한 함수에 모여 있고,
치수는 `Calendar.css`의 `.bd-week__range`가 계산합니다.
...
|---|---|---|---|
| 날짜 칸(Date 인스턴스) | 999px | 60% | 변화 없음(원) |
