...
> `get_motion_context`(`2041:2075`, recursive)가 빈 결과였던 건 사실이지만, 이 도구는 **키프레임 애니메이션만** 읽습니다.
> 변형 사이의 전환은 **프로토타입 반응(`node.reactions`)** 에 들어 있고 Plugin API(`use_figma`)로만 보입니다.

전수 집계 결과 **반응 12건**(Hover 6 + Pressed 6):
...
**⚠️ `Checked` 축에는 반응이 없고, 구현에서도 색을 전환하지 않습니다.** 흰 dot이 흰 배경에서 시작해 전환 앞부분에 보이지 않기 때문입니다(Checkbox와 같은 규칙 — `docs/INTERACTION.md` 7.3절). 미선택→선택에서 테두리가 사라지는 것도 함께 즉시 처리됩니다.
