...
- Selected 그림자(0 2px 2px `gray-900-5`)는 `box-shadow` 대신 레이어의 `filter: drop-shadow` 로 그립니다. box-shadow 는 원호를 따라 생기는데 squircle 은 원호보다 안쪽에 있어(브라우저 실측: radius 12 에서 모서리 끝 가로 최대 약 3px) 흰 배경과 그림자 사이에 틈이 생기기 때문입니다. 값과 전환(200ms)은 그대로입니다.
