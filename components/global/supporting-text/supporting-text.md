...
- **아이콘이 색상 외 두 번째 신호로 존재**: 6개 Theme 각각 스크린샷을 실측한 결과, 아이콘이 Theme마다 색상뿐 아니라 **모양 자체가 다르게 렌더링**되는 것으로 보입니다(예: Gray/Black/Brand는 정보성 원형 아이콘류, Destructed/Success/Warning은 각각 오류/체크/경고 계열로 보이는 형태). 다만 Figma 레이어명은 6개 Theme 전부 동일하게 `"Icon / Default / 12px / warning_filled"`로 되어 있어(실제 다른 SVG 애셋을 가리키는 것으로 보이나 레이어명이 갱신되지 않은 것으로 추정), **레이어명만으로는 Theme별 정확한 아이콘 식별자를 신뢰할 수 없습니다** — 실제 사용 아이콘명·의미 대응은 **확인 필요**입니다.
...
- Theme별 아이콘의 정확한 식별자(레이어명이 전부 `warning_filled`로 통일되어 있어 신뢰 불가, 6장)
