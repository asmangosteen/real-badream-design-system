...
> 적용 현황: 현재 이 규칙을 따르는 것은 **Chip** 입니다.
> 같은 구조(상태 축에서 색이 바뀌고, Figma 에 해당 축의 전환이 없음)를 가진
> Checkbox · Radio Button · Date Cell 은 아직 색을 전환합니다.
> 특히 Date Cell 의 `Pinned` 는 배경 98% ↔ 20% · 글자 2% ↔ 98% 로 같은 뒤집힘이 있어
> 전환 중 대비가 1.17 까지 떨어집니다. 통일 여부는 디자이너 확인 필요.
