...
11. **조합 컴포넌트는 자식의 속성을 좁히지 않습니다.** 바드림은 **아토믹 디자인**을 차용하고 있어서,
    조합 안에서도 자식 컴포넌트의 속성이 **그대로 살아 있어야** 합니다 —
    Text Input Group 안의 Text Input 도, 그 안의 Supporting Text 도 전부 바꿀 수 있어야 합니다.
    두 번 어겼습니다: 행 타입에서 `Omit` 으로 속성을 빼버렸고, 자식으로 가는 통로가 아예 없어
    Supporting Text 의 아이콘·글자 수 카운터를 켤 방법이 없었습니다.
    - 행 타입은 `Omit` 없이 자식 Props 그대로 (`TextInputGroupField = TextInputProps`)
    - 자식이 여럿이면 자식별 pass-through 를 둡니다 (`supportingProps` · `labelProps`)
    - `{...childProps}` 는 **기본값 뒤에** 펼쳐 바깥 값이 언제나 이기게 합니다
    - Figma 변형이 "전부 꺼진 모습" 이어도 그건 **기본값**의 근거일 뿐,
      속성을 막을 근거가 아닙니다

    **예외 — 부모가 그 축을 직접 소유할 때는 막는 게 맞습니다** (2026-09-15 디자이너 확인).
    자식이 그 축을 갖고 있더라도, **부모 안에서는 그 축의 의미가 부모에게 넘어간 경우**입니다.
    유일한 사례가 `Top Bar` 의 `mode` · `background` 입니다 —
    배경은 Top Bar 컨테이너가 통째로 칠하므로 안쪽 Navigation Bar/Status Bar 의 배경을 켜면 두 겹이 되고,
    Mode 는 상태바와 내비게이션 바의 색을 **한 화면 안에서 맞추는 값**이라 따로 놀면 안 됩니다.
    → `navProps` · `statusProps` 에서 `Omit<…, 'mode' | 'background'>` 로 **의도적으로 막아 둡니다.**
    **Top Bar 안에서는 Top Bar 설정만 따르고, 개별로 쓸 때만 각 컴포넌트의 속성을 따릅니다.**
    나머지 축(Type 8종·OS·Trailing 구성·showLabel·개별 hide 등)은 전부 그대로 열려 있어야 합니다.
    ⚠️ 이 `Omit` 은 규칙 위반이 아니라 **확정된 설계**입니다 — "규칙 11 위반"으로 되돌리지 마세요.
