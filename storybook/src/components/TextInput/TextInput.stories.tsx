    ...
    docs: {
      description: {
        component: [
          ...
          '',
          '## 입력 양식은 `inputProps` 로 줍니다',
          '',
          '실제 `<input>` 에 **네이티브 속성이 그대로** 넘어갑니다 — `inputMode`·`maxLength`·`type`·',
          '`pattern`·`autoComplete`·`name`·`required`·`aria-*` 까지.',
          '',
          '`value`·`placeholder`·`disabled` 는 Text Input이 State와 함께 관리하므로 뺐습니다(전용 prop 사용).',
          '`onFocus`/`onBlur` 는 **막지 않고 같이 호출**합니다 — 덮어쓰면 자동 State 전환이 죽습니다.',
          '',
          '> ⚠️ Figma에는 근거가 없는 영역입니다(출처: 사용자 지시).',
          '> `state` 를 고정한 진열 모드에서는 `<input>` 자체가 없어 적용되지 않습니다.',
          '',
