    ...
    docs: {
      description: {
        component: [
          ...
          '## 꼭 알아야 할 규칙 4가지',
          '',
          '**1. Typing에서는 우측 아이콘이 강제로 바뀝니다.**',
          '평소엔 교체 가능한 슬롯(`arrowhead_down`)이지만, State=Typing이 되면',
          '**`close_in_circle`(지우기 버튼)으로 강제 대체**됩니다. 자유 슬롯이 아닙니다.',
          '',
          ...
          '## State 6종',
          '',
          '| State | 테두리 | Type Box | 우측 아이콘 |',
          '|---|---|---|---|',
          '| Default | `gray-900-10%` | Placeholder | `arrowhead_down` |',
          '| Hover | `gray-900-20%` (더 진함) | Placeholder | `arrowhead_down` |',
          '| Disabled | 없음 (배경 `gray-900-5%`) | Placeholder, **`neutral/400`** | `arrowhead_down` |',
          '| Done | `gray-900-10%` | Done (값 채워짐) | `arrowhead_down` |',
          '| Selected | **브랜드 블루** | Selected (**캐럿 앞**) | `arrowhead_down` |',
          '| Typing | **브랜드 블루** | Typing (**캐럿 뒤**) | **`close_in_circle`** |',
          '',
          ...
          '',
          '## ⚠️ 확인이 필요한 것들',
          '- Figma는 **너비 280px 고정**입니다. 실제로는 화면 폭에 맞춰 늘어나는 것이 의도로 추정되나 재검증되지 않았습니다.',
  ...
  render: () => (
    <div style={{ width: 320 }}>
      {([
        ...
        ['Show Unit = True (별도 프로퍼티)', { showUnit: true }],
